...
Сейчас мы напишем простого бота кликера для известной MMORPG Lineage 2. В нём мы применим уже известные техники нам по перехвату и внедрению данных в игровое приложение.
...
Вы можете запустить скрипт и проверить, как он работает. Часто бот будет совершать не те действия, которые нужны в данный момент, потому что одно из условий было нарушено. С другой стороны, ошибки - это не критичная проблема для бота, поскольку он продолжает свою работу. Это возможно благодаря особенности команды `/target` и механизму атаки цели. Если выполнить макрос `/target` дважды, бот будет атаковать уже выбранного монстра. Таким образом он добьёт цель. Даже если монстр выжил после первой итерации цикла `while`, атака на него продолжится в следующих итерациях. Кроме того, команда "поднять предмет" не прерывает атаку, если поблизости от персонажа нет предметов. Поэтому персонаж будет продолжать бить цель и после пятисекундной задержки.

Единственная проблема, которую бот не сможет решить - это собирание предметов. Число выпадающих предметов может быть случайно и зависит от вида монстра. Поэтому иногда они будут оставаться лежать на земле. Лучшее, что можно сделать в такой ситуации - повторять действие "поднять" столько раз, сколько может максимум выпасть предметов из конкретного вида противников.

Мы можем сделать скрипт более удобным для изменений, если вынесем каждый шаг алгоритма в отдельную функцию с говорящим названием. Таким образом код станет более понятным. Результат приведён в скрипте `BlindBotFunc.au3` из листинга 2-22.
...
wend
```
Возможно, вы обратили внимание, что теперь скрипт намного лучше читается. Он начинается свою работу с вызова `Sleep`. Весь код выше - это объявления функций. Он будет выполнен только в местах их вызова, то есть в цикле `while`. Алгоритмы этого скрипта и `BlindBot.au3` идентичны.

### Бот с условиями

Попробуем улучшить нашего бота и сделать его более эффективным. Он будет реже ошибаться, если сможет проверить результат каждого своего действия. Применим функцию анализа пикселей для проверки состояния окружающих объектов.

Перед тем как мы продолжим, будет полезно добавить механизм вывода диагностических сообщений. Техника вывода сообщений в местах принятия программой важных решений известна как **трассировка** (или tracing). С помощью неё мы сможем отследить, какие решения принимает бот.
...
```
После выполнения этого скрипта в папке с ним будет создан файл `debug.log`, который содержит строку "Hello world!". Функция `LogWrite` - это обёртка над подпрограммой AutoIt `FileWrite`. Обёртка удобна, если вам понадобиться отключить вывод в лог файл. Для этого достаточно будет закоментировать в ней вызов `FileWrite`. Вы можете изменить путь до файла и его имя с помощью константы `LogFile`.

---
Вы всегда должны предусматривать способ отладки (обнаружения и устранения ошибок) вашего приложения. Самое простое решение - печатать наиболее важные решения его алгоритма.
---

Первое условие, которое бот должен проверить - это результат выбора цели. Попробуйте выделять монстров с помощью мыши самостоятельно. Заметили ли вы элемент интерфейса, который отличается при наличии и отсутствии цели? Этот элемент - окно цели. Оно появляется каждый раз, когда цель выбрана и пропадает, когда она убита или отменена. Чтобы найти это окно, мы можем применить функцию `FFBestSpot` библиотеки FastFind.
...
endfunc
```
Рассмотрим функцию `IsTargetExist` подробнее. Константы PosX и PosY - это примерные координты полосы HP цели. Мы передаём их и красный цвет полосы (равный 871D18) в функцию `FFBestSpot`. Она ищет указанную область по всему экрану. Следовательно, вместо окна цели может быть найдено окно состояния персонажа. Это может произойти независимо от наличия окна цели. Чтобы избежать этой ошибки, следует проверять координаты области, найденной функцией.

Для этой цели мы сравниваем координаты, возвращаемые `FFBestSpot` (coords[0] и coords[1]) с максимальными и минимальными допустимыми значениями (MinX/MaxX и MaxY). Эти значения определяют ожидаемую область экрана, где должно появиться окно цели. Они зависят от разрешения экрана, и вам придётся подбирать их самостоятельно.
...
wend
```
Обратите внимание на новую реализацию функций `SelectTarget` и `Attack`. Мы пытаемся выделить цель до тех пор, пока функция `IsTargetExist` не вернёт значение "true". Только после этого мы переходим в функцию `Attack`. Теперь бот атакует монстра (выбирая действие "атака" по клавише F1) до тех пор, пока тот жив.
...
Первая проблема заключается в агрессивных монстрах. Большинство из них остаются в одной и той же области карты и не реагируют на приближение игрока. Но некоторые из них в этом случае атакуют.

Наш бот выбирает цель для атаки и бежит к ней. При этом он игнорирует всех дургих сществ, которые встретятся ему по пути. Можно сказать, что они "не видимы" для бота, потому что его алгоритм их не учитывает. Таким образом, монстры могут напасть на бота, бегущего к своей цели. Он будет думать что сражается с одним противником, но на самом деле их может три или больше. Вместе они легко могут убить бота.

Мы можем решить эту проблему, если воспользуемся командой "выбор ближайшей цели". На нашей панели горячих клавиш она доступна по нажатию F10. Листинг 2-26 демонстрирует дополненную версию функции `SelectTarget`.
...
endfunc
```
Теперь бот пытается найти ближайшую цель по клавише F10. Только после того, как ему это не удалось, он применяет команду `/target`. Таким образом, мы решили проблему с "невидимыми" агрессивными монстрами.

Вторая проблема - это преграды на карте. Бот может зацепиться за камень или дерево и застрять, пока двигается к своей цели. Самое простое решение - сделать таймаут для атаки. Если отведённое на атаку время прошло, но цель осталась жива, мы можем заключить, что бот застрял. В этом случае случайные перемещения могут помочь ему обойти препятствие.

Новые версии функций `Move` и `Attack` из листинга 2-27 помогут боту обходить препятствия.
...
Возможно вы заметили дополнительное действие, появившееся в новой функции `Attack`. Это симуляция нажатия клавиши F2. Мы можем назначить любое атакующее умение на эту кнопку, и бот будет применять его в сражении. Благодаря этому он будет быстрее убивать монстров.

Теперь наш кликер может самостоятельно работать достаточно долгое время. Он умеет обходить препятствия и первым атаковать агрессивных монстров. Но есть одно улучшение, которое может сделать бота еще более живучим. Это использование зелья восстановления здоровья, которое привязано к горячей клавише F5. Чтобы правильно его применить, вам надо будет проанализировать полосу HP персонажа в окне состояния. Вы можете реализовать этот механизм самостоятельно в качестве упражнения. Алгоритм поиска области экрана будет похож на тот, которй мы реализовали в функции `IsTargetExist`.
...
2. Бот может зависнуть в некоторых непредвиденных случаях (смерть персонажа, отключение от сервера).
