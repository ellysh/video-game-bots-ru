...
MouseClick("left", 250, 300)
```
Запустив этот скрипт, вы заметите, что изменились координаты чёрной точки, которую он рисует в окне Paint. Выбранный нами режим обеспечивает более точное позиционирование курсора. При разработке кликеров предпочтительнее использовать именно его. Он одинаково хорошо работает для окон в обычном и полноэкранном режиме. Единственный недостаток этого режима - сложность отладки скриптов. Утилиты вроде ColorPix отображают только абсолютные координаты пикселей.

Перетаскивание (или drag-and-drop) - одно из распространенных действий мышью в компьютерных играх. Для его симуляции autoIt предоставляет функцию `MouseClickDrag`. Блок кода 2-8 демонстрирует её использование.
