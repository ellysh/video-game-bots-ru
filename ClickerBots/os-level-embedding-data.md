...
Функция AutoIt `Send` симулирует нажатия клавиш в активном окне. Другими словами, вы не можете свернуть это окно или переключится на другое, что в некоторых случаях неудобно. Функция `ControlSend` позволяет обойти такое ограничение. Мы можем переписать скрипт `Send.au3` с использованием `ControlSend`, как демонстрирует блок кода 2-3.
...
Мы воспользуемся графическим редактором Microsoft Paint для тестирования скриптов, симулирующих действия мыши. Самое простое действие - это однократный щелчок в указанной точке экрана. Блок кода 2-6 демонстрирует соответствующий скрипт.
...
Функция AutoIt `Opt`, вызванная с параметром `MouseCoordMode`, позволяет выбрать режим координат для текущего скрипта. Блок кода 2-7 демонстрирует выбор координат относительно клиентской части окна в скрипте `MouseClick.au3`.

**Блок кода 2-7.** *Изменённый скрипт `MouseClick.au3`*
```AutoIt
...
Перетаскивание (или drag-and-drop) - одно из распространённых действий мышью в компьютерных играх. Для его симуляции AutoIt предоставляет функцию `MouseClickDrag`. Блок кода 2-8 демонстрирует её использование.
 
**Блок кода 2-8.** *Скрипт `MouseClickDrag.au3`*
```AutoIt
$hWnd = WinGetHandle("[CLASS:MSPaintApp]")
WinActivate($hWnd)
...
AutoIt предоставляет функцию `ControlClick`, которая симулирует щелчок мыши в неактивном окне. Пример её использования приведён в блоке кода 2-9.

**Блок кода 2-9.** *Скрипт `ControlClick.au3`*
```AutoIt
$hWnd = WinGetHandle("[CLASS:MSPaintApp]")
