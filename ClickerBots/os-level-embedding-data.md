...
4 + (2 + 2 + 4 + 4 + 8) + 4 = 28
```
Слагаемые в скобочках - это размеры полей вложенной структуры `KEYBDINPUT`. Вопрос вызывают последние 4 байта. Возможно вы обратили внимание, что объявленная в скрипте `Send.au3` структура `INPUT` имеет последнее поле типа `dword` для выравнивания.
...
Функция AutoIt `Send` симулирует нажатие клавиш в активном окне. Это означает, что вы не можете свернуть это окно или переключится на другое. В некоторых случаях это неудобно. Функция `ControlSend` позволяет обойти это ограничение.
