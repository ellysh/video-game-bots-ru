...
Если вы переместите окно Notepad так, чтобы перекрыть им точку с координатой x = 200, y = 200 рабочего стола, результат возвращаемый скриптом `PixelGetColor.au3` изменится. Это означает, что он анализирует не конкретное окно, а весь рабочий стол.
...
Сравним WinAPI вызовы скриптов `PixelGetColorWindow.au3` и `PixelGetColor.au3`, перехваченные с помощью приложения API Monitor. В обоих случаях функция `GetDC` получает "NULL" входным параметром. Такое поведение похоже на ошибку в реализации функции `PixelGetColor` версии 3.3.14.1 AutoIt. Возможно, она будет исправлена в следующий версиях. Попробуем обойти эту ошибку.
...
Проверим, какие WinAPI вызовы делает функция `PixelSearch`. Для этого запустим скрипт `PixelSearch.au3` из приложения API Monitor. Подождём, пока он отработает. После этого будем искать текст "0, 207" в окне "Summary". Вы должны найти вызов WinAPI `StretchBlt`, как показано на иллюстрации 2-9.
...
Следующий шаг AutoIt функции `PixelSearch` – это WinAPI вызов `GetDIBits`. Он конвертирует аппаратно-зависимое битовое изображение (DDB) в **аппаратно-независимое** (DIB). Зачем это нужно? DIB формат более удобен, потому что позволяет работать с изображениями как с обычным массивом.
...
Теперь вы можете вызывать функции FastFind.

Чтобы работать с библиотекой из C++ приложения, надо сделать следующее:

1. Скачать и установить C++ компилятор. Это может быть IDE **Visual Studio Community** с сайта Microsoft, в которую уже встроен компилятор. Альтернативное решение – набор инструментов [**MinGW**](https://nuwen.net/mingw.html).
...
Вторая задача, которую решает FastFind, – это обнаружение изменений изображения на экране. Функция `FFLocalizeChanges` реализует подходящий алгоритм. Для демонстрации её работы воспользуемся окном приложения Notepad.
...
1. Запустить приложение Notepad и развернуть его окно на весь экран.

2. Запустить скрипт.

3. Переключиться на окно Notepad.

4. Ожидать диалогового окна с сообщением "Измените изображение".

5. Ввести несколько символов в Notepad в течении пяти секунд.
...
Для демонстрации возможностей ImageSearch найдём иконку Notepad в заголовке его окна. Прежде всего необходимо подготовить фрагмент изображения, который надо найти. В нашем случае это иконка, приведённая на иллюстрации 2-11.

![Иконка Notepad](notepad-logo.bmp)

**Иллюстрация 2-11.** *Иконка Notepad*

Вы можете создать эту иконку с помощью приложения Paint. Для этого надо запустить приложение Notepad, сделать скриншот, вставить его в Paint и вырезать иконку. Сохраните результат в файл с именем `notepad-logo.bmp` в папку с проектом `ImageSearchDemo`.
