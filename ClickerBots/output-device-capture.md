...
```
Простейшая функция AutoIt для чтения цвета пикселя - это `PixelGetColor`. Её входные параметры - это координаты пикселя по осям X и Y. Функция возвращает код цвета в **десятичной системе счисления**. Листинг 2-10 демонстрирует её использование.
