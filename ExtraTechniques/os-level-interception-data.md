...
5. Первая инструкция функции `TextOutA` модуля `gdi32.dll`, которая была заменена на `JMP`, восстанаваливается в исходное значение. Затем она вызывается с помощью  инструкции `CALL`.

6. После выполнения функция `TextOutA` возвращает управление обратно в модуль `handler.dll` с помощью инструкции `RETN`.

7. Первая инструкция `TextOutA` снова замещается на `JMP`, которая переходит в обработчик `TextOutA_handler` модуля `handler.dll`.
