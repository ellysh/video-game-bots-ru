...
Рассмотрим параметры командной строки примера `CTest.exe`. Первый из них `exec TestApplication.exe` указывает целевое приложение, которое следует запустить. После запуска в память его процесса будет загружена DLL библиотека с обработчиками вызовов. Второй параметр `-log=out.txt` указывает текстовый файл для вывода информации о перехваченных вызовах.
