...
Проблема решается с помощью [**Import Table**](http://sandsprite.com/CodeStuff/Understanding_imports.html) (таблица импорта). Кроме неё есть так называемая **Thunk Table** (таблица переходов). Эти таблицы часто путают. Рассмотрим подробнее их внутренее устройство.
...
Некоторые компиляторы генерируют код, не использующий Thunk Table. Благодаря этому, удаётся избежать накладных расходов, связанных с дополнительным `JMP` переходом. Код, сгенерированный MinGW компилятором, использует Thunk Table. В этом случае cхема вызова импортируемой функции `TextOutA` будет соответствовать иллюстрации 5-1.
