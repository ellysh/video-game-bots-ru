...
Вторая техника перехвата вызовов приложения, которую мы рассмотрим, заключается в модификации системных функций. Предположим, что PE-загрзучик прочитал `gdi32.dll` библиотеку в память процесса. С этого момента, получив доступ к этой памяти, мы можем модифицировать функции gdi32 модуля DLL, которые следует прехватить. Модификация касается только первой ассемблерной инструкции. Мы заменяем её на переход в наш код.
...
Иллюстрация 5-3 демонстрирует порядок вызова функции `TextOutA` после модификации WinAPI.
