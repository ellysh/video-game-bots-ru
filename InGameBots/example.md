...
4. Введите адрес строки с именем персонажа в поле "Enter address expression" диалога поиска. В моём случае это адрес 04FC00D. Нажмите кнопку "Follow expression". Теперь курсор в окне с дампом памяти указывает на первый байт строки.
...
```
Эта инструкция сравнивает константу 4 и число типа DWORD, хранящееся по адресу "ESI + 4".
