...
Проблема бота в том, что он нажимает только одну горячую клавишу из четырёх доступных. Из-за этого персонаж не будет использовать все зелья лечения, которые у него есть. Новая версия цикла проверки параметра здоровья приведена в листинге 3-14.
...
```
Теперь мы храним список горячих клавиш в байтовом массиве `keys`. Для индексации этого массива используется переменная `keyIndex`. Она инкрементируется после каждого использования зелья лечения. Если `keyIndex` достигает конца массива, она сбрасывается в ноль. Таким образом бот будет использовать все слоты панели горячих клавиш. Когда зелья лечения в первом ряду панели закончатся, бот перейдёт ко второму ряду и т.д.

Ещё одно улучшение бота - это контроль за уровнем маны персонажа. Для этого подойдёт тот же самый алгоритм, что и для контроля уровня здоровья. Для восстановления маны, бот может использовать специальное зелье.
...
```
Запись нового значения параметра персонажа происходит через WinAPI функцию `WriteProcessMemory`. Для удобства работы с ней используется обёртка `WriteWord`. Теперь если уровень здоровья персонажа становится меньше 100, бот записывает его значение 100 прямо в память процесса. У этого подхода есть один серьёзный недостаток - он нарушает игровую механику. Бот меняет параметр объекта в обход алгоритмов самой игры. По этой причине состояние игровых объектов может стать противоречивым.

Попробуйте протестировать версию бота из листинга 3-15. В большинстве случаев уровень здоровья персонажа не будет меняться после записи нового значения. Причина такого поведения в том, что приложение хранит этот параметр в нескольких местах (не только в объекте персонажа, который мы нашли). Скорее всего, есть некоторый алгоритм для контроля параметров. Если одна из копий значения параметра отличается от остальных, она будет скорректирована. Точно так же игровой сервер следит за корректностью данных в онлайн играх. Поэтому рассмотренный нами подход работает только в простых играх без дополнительных алгоритмов контроля.

Есть ещё один способ встраивания данных в память процесса игрового приложения. Он основан на техниках внедрения кода, описаных в следующих статьях:
...
Алгоритм нашего бота очень простой. Он автоматизирует использование зелий лечения, и игрок может на них не отвлекаться. Возможно ли написать более сложного бота, который бы самостоятельно убивал монстров? Эта задача выполнима. Самым сложным для бота будет поиск объектов монстров в памяти игрового процесса.

Мы знаем, где хранятся координаты X и Y игрового персонажа (см. таблицу 3-6). Они хранится в двухбайтовых числах, следующих друг за другом в памяти. Скорее всего координаты других игровых объектов хранятся в таком же формате.

Теперь предположим, что когда монстр атакует персонажа, они находятся рядом друг с другом, и их координаты отличаются незначительно. Бот мог бы сканировать память игрового процесса на наличие следующих друг за другом двухбайтовых чисел, значения которых близки к текущим координатам игрового персонажа. Следующий шаг - отсеивание ложных результатов. Подсказкой для алгоритма может быть то, что координаты всех видимых на экране монстров должны находиться в одном и том же сегменте памяти.

Бот может запомнить сегмент в котором хранятся найденные координаты монстров. После этого он может искать только в этом сегменте. Для атаки монстров бот может симулировать действия клавиатуры или мыши с помощью WinAPI функции `PostMessage`.

## Выводы

Мы реализовали простого внутриигрового бота для Diablo 2. Он использует характерные для своего типа техники взаимодействия с игрой. Рассмотрим его достоинства и недостатки. Мы можем обобщить их на любого внутриигрового бота.

Преимущества:

1. Бот получает точную информацию о состоянии игровых объектов. Ошибки и неточности (как в кликерах0 крайне маловероятны.

2. Есть несколько способов встраивать действия бота в процесс игрового приложения: симулировать действий игрока, прямая запись значений в память процесса, вызов внутренних функций игры. Из них можно выбрать наиболее подходящий вариант.

3. Бот способен очень быстро реагировать на события в игре. Зачастую скорость его реакции может быть выше, чем у игрока.
