...
```
Приложение из листинга 3-1 подключается к процессу с PID равным 1804. Вам нужно заменить его на PID запущенного в данный момент процесса. Узнать идентификаторы запущенных процессов можно с помощью приложения Task Manager (диспетчер задач). Укажите PID целевого процесса в следующей строке:
```C++
    DWORD pid = 1804;
```
...
5. Отлючите WinDbg от процесса Notepad с помощью команды `.detach`.
...
Мы рассмотрели метод чтения базового адреса сегмента динамической памяти по умолчанию из структуры `PEB`. Однако, у процесса может быть несколько таких сегментов. К ним можно получить доступ через WinAPI функции. Они позволяют перебрать все сегменты динамической памяти указанного процесса. Алгоритм очень похож на перебор активных потоков в снимке состояния системы.
