...
В интерфейсе WinAPI есть ещё одна функция для обнаружения отладчика - `CheckRemoteDebuggerPresent`. Принцип её работы такой же как и `IsDebuggerPresent`. Она позволяет обнарживать отладчик, подключённый к указанному процессу.
