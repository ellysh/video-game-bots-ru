...
2. Скрипт `TestTcpReceiver.py` запускает цикл ожидания входящего пакета через открытый им сокет. Говорят, что скрипт **слушает** (listen) порт 24000 в ожидании соединения.
...
3. Клиент отвечает пакетом (номер 12) с установленным флагом ACK. Его ack номер, равный единице, подтверждает seq сервера. После этого шага клиент и сервер подтвердили свои seq номера и готовы к взаимодействию. Следующий фильтр отображает ответ клиента:
```
...
Возможно вы заметили, что в последнем фильтре для ответа клиента мы проверяем значаение флага PUSH. Если этот флаг установлен в единицу, это означает что пакет содержит данные, отправленные приложением. Вы можете инвертировать условие, чтобы отобразить только эти пакеты:
```
tcp.flags.push==1
```
Если вы хотите прочитать данные, переданные нашим тестовым приложением, выделите пакет под номером 13 с установленным в единицу флагом PUSH. Затем щелкните левой кнопкой мыши по пункту "Data" (данные) в списке заголовков. В результате в нижней части окна Wireshark синим цветом будут выделены соответсвующие байты пакета, как на иллюстрации 4-8.

![Пакет с данными](Figure_4-8.png)

**Иллюстрация 4-8.** *Пакет с данными*

Тестовое приложение передаёт три байта, которые в шестнадцатеричной системе равны 2C, 37, 42. Если перевсти эти числа в десятичную, систему получим: 44, 55, 66. Вы можете удостовериться в листинге 4-2, что именно эти три байта передаёт скрипт `TestTcpSender.py`.

Вы могли заметить на иллюстрации 4-6, что пакет с номером 14, следующий за передачей данных, имеет ack номер равный четырём. Что это значит? После установки соединения номера seq и ack используются для подтверждения числа байт данных, полученных сервером от клиента. Следовательно, когда сервер полчает данные, он отвечает пакетом со следующим ack номером:
```
...
Иллюстрация 4-9 демонстрирует последовательность PUSH пакетов переданных клиентом на сервер. В столбце `Info` вы можете проследить увеличение номеров ack и seq. Каждый пакет с подтверждением от сервера имеет ack, рассчитанный по рассмотренной выше формуле.

![Последовательность TCP пакетов](Figure_4-9.png)

**Иллюстрация 4-9.** *Последовательность TCP пакетов*

Обратите внимание, что клиент всегда посылает свои пакеты на целевой порт 24000. Порт отправителя равен 35936 на иллюстрации 4-9, и 32978 на иллюстрации 4-6. Как вы помните, ОС назначает порт клиенту каждый раз, когда он пытается установить новое соединение. Номер порта выбирается случайным образом, и его невозможно предсказать. Поэтому для фильтрации пакетов в Wireshark лучше проверять порт сервера, а не клиента.

Вернёмся к иллюстрации 4-6, на которой приведен TCP трафик для передачи одного пакета данных. После получения данных, сервер отправляет пакет номер 14 с подтверждением. Затем следуют три пакета с номерами 15, 16 и 17 для закрытия TCP соединения. Этот процесс состоит из следующих шагов:

1. Клиент отправляет пакет номер 15, в котором установлен флаг FIN. Таким образом хост запрашивает разрыв соединения. В нашем случае в этом пакете также установлен флаг ACK. С его помощью клиент подтверждает получение от сервера пакета номер 14, seq номер которого равен 1. Чтобы отобразить только этот пакет в Wireshark, примените следующий фильтр:
```
tcp.flags.fin==1 and tcp.dstport==24000
```

2. Сервер отвечает пакетом номер 16, в котором установлены флаги FIN и ACK. Его номер ack равен пяти, т.е. номер seq клиент плюс один. Теперь флаг ACK означает, что сервер подтверждает получение FIN пакета от клиента. С помощью флага FIN сервер просит клиента закрыть соединение на своей стороне. Фильтр для отображения этого пакета следующий:
```
tcp.flags.fin==1 and tcp.srcport==24000
```

3. Клиент отвечает пакетом номер 17 с установленным флагом ACK. Он означает, что запрос сервера на закрытие соединения получен. Номер seq этого пакета равен номеру ack последнего пакета (номер 16) от сервера. Фильтр для отображения:
```
...
Мы рассмотрели тестовое приложение, которое передаёт данные по TCP протоколу. Теперь мы знаем, как перехватить и проанализировать такой вид трафика. Однако, многие онлайн игры используют UDP протокол вместо TCP.
...
3. Скрипт `TestUdpSender.py` (из листинга 4-4) запускается вторым. Он открывает UDP сокет и привязывает его к порту 24001 и адресу localhost. Последний шаг необязателен. ОС может назначить произвольный порт отправителю UDP пакетов. Однако, это может быть полезным, если понадобится передавать данные в обоих направлениях.
...
Как вы видите, алгоритм тестового приложения стал проще, по сравнению с использованием TCP протокола. Нет необходимости устанавливать и разрывать соединение. Всё, что делает приложение - это отправляет единственный пакет с данными.
...
Запустите Wireshark и начните перехват трафика на loopback интерфейсе. После этого запустите скрипты `TestUdpReceiver.py` и `TestUdpSender.py`. Вы должны получить результат, изображённый на иллюстрации 4-10.

Если вы видите несколько пакетов в списке перехваченных Wireshark, примените следующий фильтр:
```
udp.port==24000
```
Вы должны увидеть единственный пакет, содержащий три байта данных: 2C, 37, 42.

![Перехваченный UDP пакетов](Figure_4-10.png)
