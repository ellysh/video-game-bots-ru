...
Первая категория алгоритмов в списке использется для создания [**секретного ключа**](https://ru.wikipedia.org/wiki/Ключ_(криптография)), который удовлетворяет требованиям [**шифра**](https://ru.wikipedia.org/wiki/Шифр).
...
Иентерфейс методов `encrypt` и `decrypt` класса `DES3Cipher` такой же, как и у `XORCipher`. Первый принимает на вход открытый текст, а второй - шифротекст.
