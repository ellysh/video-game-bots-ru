...
_**Иллюстрация 3-15.** Скриншот окна Diablo 2_
...
_**Иллюстрация 3-16.** Параметры игрового персонажа_
...
_**Иллюстрация 3-17.** Результаты поиска параметра игрового персонажа_
...
Почему последовательность байт "9E 36 FF 10" равна числу 285161118? Мы запускаем Diablo 2 на процессоре с архитектурой x86, которая имеет [**порядок байт**](https://ru.wikipedia.org/wiki/%D0%9F%D0%BE%D1%80%D1%8F%D0%B4%D0%BE%D0%BA_%D0%B1%D0%B0%D0%B9%D1%82%D0%BE%D0%B2) от младшего к старшему (little-endian byte order). Следовательно, значение из окна Memory Viewer нужно перевернуть, чтобы получить правильно число. Другими словами, последовательность байтов "9E 36 FF 10" надо интерпретировать как "10 FF 36 9E". Вы можете воспользоваться стандартным приложением Windows Calculator, чтобы перевести число 10FF369E в десятичную систему и получить 285161118.

![Memory Viewer](memory-viewer.png)

_**Иллюстрация 3-18.** Окно Memory Viewer сканера Cheat Engine_
...
_**Иллюстрация 3-19.** Исследование изменений в памяти процесса Diablo 2_
...
_**Таблица 3-6.** Найденные параметры игрового объекта_
...
_**Иллюстрация 3-20.** Конфигурация Cheat Engine_
...
_**Иллюстрация 3-21.** Имя персонажа в памяти процесса_
...
2. Щёлкните правой кнопкой мыши в левом нижнем окне OllyDbg и переключитесь на шестнадцатеричный формат [**дампа памяти**](https://ru.wikipedia.org/wiki/%D0%94%D0%B0%D0%BC%D0%BF_%D0%BF%D0%B0%D0%BC%D1%8F%D1%82%D0%B8).
...
_**Иллюстрация 3-22.** Точка останова на начале объекта игрового персонажа_
...
Последовательность неизменных байтов в начале объекта можно использовать как цель для поиска. Назовём её [**магическим числом** или сигнатурой](https://ru.wikipedia.org/wiki/%D0%9C%D0%B0%D0%B3%D0%B8%D1%87%D0%B5%D1%81%D0%BA%D0%BE%D0%B5_%D1%87%D0%B8%D1%81%D0%BB%D0%BE_%28%D0%BF%D1%80%D0%BE%D0%B3%D1%80%D0%B0%D0%BC%D0%BC%D0%B8%D1%80%D0%BE%D0%B2%D0%B0%D0%BD%D0%B8%D0%B5%29) Учтите, что в вашем случае эта последовательность будет отличаться.
...
_**Иллюстрация 3-23.** Поиск объекта игрового персонажа в памяти процесса Diablo 2_
...
_**Листинг 3-11.** Код подключения к процессу_
```C++
int main()
{
    Sleep(4000);
...
_**Листинг 3-12.** Код поиска игрового объекта в памяти процесса_
```C++
...
_**Листинг 3-13.** Код проверки уровня здоровья персонажа_
```C++
...
_**Таблица 3-7.** Параметры функции `PostMessage`_
...
_**Листинг 3-14.** Использование всех слотов панели горячих клавиш_
```C++
    ULONG hp = 0;
...
_**Листинг 3-15.** Запись нового значения параметра персонажа в память процесса_
```C++
