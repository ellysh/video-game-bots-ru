...
{caption: "Иллюстрация 3-24. Точка останова в `main` функции"}
...
{caption: "Иллюстрация 3-25. Сегменты модуля TestApplication"}
...
{caption: "Таблица 3-8. Значения флагов сегмента `.data`"}
...
2. Нажмите комбинацию клавиш Ctrl+N, чтобы открыть окно "Names in TestApplication" (имена в TestApplication). Перед вами [**таблица символов**](https://ru.wikipedia.org/wiki/Таблица_символов) тестового приложения, в которой указаны все его глобальные переменные, константы и функции.
...
{caption: "Иллюстрация 3-26. Изменение значения регистра EAX"}
...
{caption: "Иллюстрация 3-27. Диалог редактирования инструкции"}
...
Что означает замена инструкции `JE` на `JNE`? Рассмотрим код C++, соответствующий каждому варианту. Исходная инструкция `JE` аналогична следующему оператору `if`:
```C++
if (IsDebuggerPresent())
{
    printf("debugger detected!\n");
...
{caption: "Иллюстрация 3-28. Взаимодействие родительского и дочернего процессов"}
...
{caption: "Иллюстрация 3-29. Инструкции функции `IsDebuggerPresent`"}
...
2. Если встраиваемая функция содержит [**рекурсивные вызовы**](https://ru.wikipedia.org/wiki/Рекурсия#В_программировании), т.е. вызывает саму себя.
...
{caption: "Иллюстрация 3-30. Диалог редактирования флага "BeingDebugged"}
...
В машинном коде инструкция `INT 3` представляется шестнадцатеричным числом 0xCC. В результате поиска OllyDbg вы получите список инструкций, содержащих 0xCC в своём [**коде операции**](https://ru.wikipedia.org/wiki/Код_операции) (opcode). Далеко не все из этих инструкций являются `INT 3`, но вам придётся их проверить.
...
Наша реализация шифра XOR упрощена в целях демонстрации подхода. Если вы планируете использовать её для защиты своих приложений, её следует доработать. Прежде всего будет полезно поместить алгоритм шифрования в [шаблон класса](https://ru.wikipedia.org/wiki/Шаблоны_C++) (template) C++. Для этого класса следует определить арифметические операторы и присваивание. Тогда вы сможете шифровать данные неявно, и код будет выглядеть намного компактнее. Например так:
```C++
