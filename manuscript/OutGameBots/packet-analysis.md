...
3. Нажмите кнопку "Details..." (сведения). Откроется окно "Network Connection Details" (сведения о сетевом подключении), в котором указан адрес IPv4.
...
{caption: "Листинг 4-3. Скрипт `TestUdpReceiver.py`", format: Python}
